...
void Controller::startFind(QString dir, QString file)
{
   ...
   connect(findThread.data(), SIGNAL(stopedThread()), this, SLOT(deleteThread()));
   connect(findThread.data(), SIGNAL(writeFoundPath(QString)), this, SLOT(printCurrentPath(QString)));
   connect(findThread.data(), SIGNAL(findFile(QString)), this, SLOT(genStringPathFile(QString)));
   emit newFind();
}
